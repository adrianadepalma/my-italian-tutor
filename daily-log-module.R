...
dailySummaryServer <- function(
  ...
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$generate_log, {
      ...
      if (
        is.null(session_log) || length(reactiveValuesToList(session_log)) == 0
      ) {
      ...
      for (step_name in names(log_list)) {
        ...
        if (!is.list(step_data)) {
          if (!is.null(step_data) && nzchar(as.character(step_data))) {
            has_content <- TRUE
          }
        ...
        if (
          !is.null(step_data$conversations) &&
            length(step_data$conversations) > 0
        ) {
          for (conv in step_data$conversations) {
            # Defensive: check for required fields and type
            if (!is.list(conv)) {
              if (!is.null(conv) && nzchar(as.character(conv))) {
                has_content <- TRUE
              }
              local_log <- paste0(local_log, as.character(conv), "\n")
              next
            }
            scenario <- if (!is.null(conv$scenario)) {
              conv$scenario
            } else {
              "(no scenario)"
            }
            student_role <- if (!is.null(conv$student_role)) {
              conv$student_role
            } else {
              "You"
            }
            assistant_role <- if (!is.null(conv$assistant_role)) {
              conv$assistant_role
            } else {
              "Assistant"
            }
      ...
      prompt_summary <- paste(
        "Review all exercises completed during this session.",

        "\n\nWrite a concise learner summary in English.",
        ...
        "\n\nOnly identify weaknesses that appear more than once.",
      ...
      summary_text <- tryCatch(
        call_openai(prompt_summary, model = summary_model),
    ...
    output$daily_log_out <- renderText({
      daily_log_text()
    })
  })
}
